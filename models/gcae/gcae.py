...
class GCAE(nn.Module):
    ...
    def decode(self, z, x_size, x_ref=None):
        ...
        for ind, (layer_, importance) in enumerate(zip(self.st_gcn_dec, self.ei_dec)):
            # print('GCAE', type(layer_), x.shape, self.A.shape)
    ...
    def build_dec(self, dec_ch_fac, dec_stride, dec_1st_residual=True):
        ...
        for i in range(1, len(dec_ch_fac)):
            ...
            st_gcn_dec.append(ConvBlock(dec_ch_fac[i - 1] * self.h_dim, dec_ch_fac[i] * self.h_dim, self.kernel_size, 1, headless=self.headless))
...
class Encoder(nn.Module):
    ...
    def pred(self, x):
        reco_graph, input_feature_graph = self.model(x)

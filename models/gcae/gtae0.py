...
class GTAE(nn.Module):
    def __init__(self, in_channels, ff_hidden_size=64, num_self_att_layers=2, nhead=4, 
        ...
        for _layer in range(self.num_GNN_layers_encoder):
            encoder_layers = TransformerEncoderLayer(d_model=self.feature_dim_size, nhead=self.nhead, \
                                                     dim_feedforward=self.ff_hidden_size, dropout=dropout) # Default batch_first=False (seq, batch, feature)
            self.ugformer_encoder.append(TransformerEncoder(encoder_layers, self.num_self_att_layers))
            self.lst_gnn_encoder.append(GraphConvolution(self.feature_dim_size, self.feature_dim_size, act=torch.relu))
            self.tcn_encoder.append(nn.Sequential(
                nn.Conv2d(self.feature_dim_size, self.feature_dim_size,
                       (temporal_kernelsize, 1), (arch_dict['enc_stride'][_layer], 1), temporal_padding),
                nn.BatchNorm2d(self.feature_dim_size), 
                nn.ReLU(),
                nn.Dropout(dropout)))
        ...
        for _layer in range(self.num_GNN_layers_decoder):
            if arch_dict['dec_stride'][_layer] != 1:
            ...
            encoder_layers = TransformerEncoderLayer(d_model=self.feature_dim_size, nhead=self.nhead, \
                                                     dim_feedforward=self.ff_hidden_size, dropout=dropout)  # Default batch_first=False (seq, batch, feature)
            self.ugformer_decoder.append(TransformerEncoder(encoder_layers, self.num_self_att_layers))
            self.lst_gnn_decoder.append(GraphConvolution(self.feature_dim_size, self.feature_dim_size, act=torch.relu))
            self.tcn_decoder.append(nn.Sequential(
                nn.Conv2d(self.feature_dim_size, self.feature_dim_size,
                          (temporal_kernelsize, 1), 1, temporal_padding),
                nn.BatchNorm2d(self.feature_dim_size),
                nn.ReLU(),
                nn.Dropout(dropout)))
    ...
    def encode(self, x):
        ...
        for layer_idx in range(self.num_GNN_layers_encoder):
            ...
            # print(3, x.shape)
            x = self.lst_gnn_encoder[layer_idx](x, torch.sum(self.A, dim=0))
            # print(4, x.shape)
            x = self.tcn_encoder[layer_idx](x)
            # print(5, x.shape)
...
class TCN(nn.Module):
    def __init__(self, in_channels, out_channels, temporal_kernelsize, dropout):
        super(TCN).__init__()
        temporal_padding = ((temporal_kernelsize - 1) // 2, 0)
...
class CommonTransformer(nn.Module):
    def __init__(self, feature_dim_size, nhead, ff_hidden_size, num_self_att_layers, dropout):
        super(CommonTransformer).__init__()
